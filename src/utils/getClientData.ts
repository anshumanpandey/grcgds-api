...
export const getClientData = async ({ id, brokerId, clientAccountCode }: { id?: string | number, brokerId?: string | number, clientAccountCode?: string | number }): Promise<ClientData> => {
    const query = DB?.select({
      ...
      trustpilotEmail: "clients.trustpilotEmail",
      trustpilotPassword: "clients.trustpilotPassword",
      ...
      .leftJoin(
        "data_suppliers_user",
        "data_suppliers_user.clientId",
        "clients.id"
      )
      .joinRaw(
        'LEFT JOIN broker_account_type on data_suppliers_user.account_type_code and broker_account_type.name = "Prepaid Standard" '
      );
