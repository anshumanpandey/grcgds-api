...
export default async ({
  ...
  const pickupLocation =
    rcBooking.OTA_VehRetResRS.VehRetResRSCore[0].VehReservation[0].VehSegmentInfo[0].LocationDetails.find(
      (e) => e.CodeContext[0] == "Pickup Location"
    );
  const dropLocation =
    rcBooking.OTA_VehRetResRS.VehRetResRSCore[0].VehReservation[0].VehSegmentInfo[0].LocationDetails.find(
      (e) => e.CodeContext[0] == "Return Location"
    );
  const pickupDateString =
    rcBooking.OTA_VehRetResRS.VehRetResRSCore[0].VehReservation[0]
      .VehSegmentCore[0].VehRentalCore[0].PickUpLocation[0].Pickupdate[0];
  const dropDateString =
    rcBooking.OTA_VehRetResRS.VehRetResRSCore[0].VehReservation[0]
      .VehSegmentCore[0].VehRentalCore[0].ReturnLocation[0].Returndate[0];
  const extraKeys = Object.keys(
    rcBooking.OTA_VehRetResRS.VehRetResRSCore[0].VehReservation[0]
      .VehSegmentCore[0].Extras[0]
  );

  const pickCountryCode =
    pickupLocation?.Address[0]?.CountryName?.[0]?.Code[0] || "";
  const dropCountryCode =
    dropLocation?.Address[0]?.CountryName?.[0]?.Code[0] || "";
  ...
  return {
    customer: {
      firstname:
        rcBooking.OTA_VehRetResRS.VehRetResRSCore[0].VehReservation[0]
          .Customer[0].Primary[0].PersonName[0].GivenName[0],
      lastname:
        rcBooking.OTA_VehRetResRS.VehRetResRSCore[0].VehReservation[0]
          .Customer[0].Primary[0].PersonName[0].Surname[0],
      email:
        rcBooking.OTA_VehRetResRS.VehRetResRSCore[0].VehReservation[0]
          .Customer[0].Primary[0].Email[0],
      address: {
        addressLine:
          rcBooking.OTA_VehRetResRS.VehRetResRSCore[0].VehReservation[0]
    ...
    extras:
      extraKeys.map((key) => {
        return {
          key,
          val: rcBooking.OTA_VehRetResRS.VehRetResRSCore[0].VehReservation[0]
            .VehSegmentCore[0].Extras[0][key][0],
        };
      }) || [],
    taxData: {
      total:
    ...
    supplier: {
      ...
      id: `GRC-${supplierData.brokerId}0000`,
      name: "Right Cars",
      rateId: supplierData.account_code,
    },
    pickupLocation: {
